...
public class EIPurchaseConsumer
       implements ExceptionListener, MessageListener
{
    ...
    public EIPurchaseConsumer(String[] args)
    {
        parseArgs(args);
        ...
        /* print parameters */
        System.err.println("\n------------------------------------------------------------------------");
        System.err.println("EIPurchaseConsumer");
        System.err.println("------------------------------------------------------------------------");
    ...
    void usage()
    {
        System.err.println("\nUsage: EIPurchaseConsumer [options] [ssl options]");
        System.err.println("");
    ...
    public void onMessage(Message message) {
        try {
            if (message instanceof TextMessage) {
                ...
                // Simply print out a line to the output screen
                System.out.println("Received request for stock replenishment");
                System.out.println("\tTime:       " + System.currentTimeMillis() + " ms");
                System.out.println("\tContents:   " + textMessage.getText());

                // Approval from manager
                System.out.print("Approve this replenishment? (Y/n): ");
                ...
                try {
                   ...
                   if (approval.equalsIgnoreCase("y")) {
                     String reorderText = textMessage.getText();

                     // This is to format the text to remove the "approval for"
                     // in the sentence to make more sense
                     reorderText = reorderText.replaceFirst("approval for ", "");
                     String args[] = {"-server", serverUrl, "-queue", "reorder.request", reorderText};

                      // Instantiate a generic message producer to send a message to supplier system
                      EIGenericMsgProducer msgProducer = new EIGenericMsgProducer(args);
                   } else {
                     // ignore low stock replenishment request
                     System.out.println("Rejected : Not processing replenishment request");
                   }
                } catch (IOException ioe) {
                   System.out.println("IO error trying to get your approval.");
                   System.exit(1);
                }
